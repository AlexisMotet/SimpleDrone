...
class BasicIMU:
    ...
    def read(self, t: float, accel_world: NDArray[np.float64], 
             quat_world2body: NDArray[np.float64], ang_vel_body: NDArray[np.float64]):
        
        if t - self.last_t  < self.sample_period:
            return self.accel_meas, self.gyro_meas
        self.last_t = t

        R_world2body = quat.to_rotation_matrix(quat_world2body) # Is it yaw pitch roll?
