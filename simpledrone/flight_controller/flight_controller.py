import numpy as np
from simpledrone.transferdata import DroneState
...
class IMU:
    ...
    def __init__(self, accel_noise_ug_rtHz: float = 70.0, gyro_noise_mdps_rtHz: float = 3.8, bandwidth: float = 100.0, odr: float = 1000.0):
        ...
        self.last_read_date = 0.0

    def read(self, accel, angular_vel, date):
        dt = date - self.last_read_date
        if dt < self.sample_period:
            return self.accel, self.gyro
        self.last_read_date = date
        self.accel = accel + np.random.normal(0.0, self.accel_std, 3)
        self.angular_vel = angular_vel + np.random.normal(0.0, self.gyro_std, 3)
        return self.accel, self.angular_vel
...
class Mahony:
    ...
    def update(self, accel, angular_vel, dt):
        gx, gy, gz = gyro
        ax, ay, az = accel
        q = self.quaternion
...
class FlightController:
    ...
    def update(self, dt, drone_state: DroneState, rc_cmd: Tuple[float, float, float, float], dt: float) -> np.ndarray:
